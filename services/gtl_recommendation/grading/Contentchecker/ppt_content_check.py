...
from services.gtl_recommendation.grading.Contentchecker.prompt import build_pptx_contentcheck_prompt

logger = get_custom_logger(__name__)


class PptxContentChecker:
    ...
    def _clean_text(self, textContent: str) -> str:
        """Clean extracted PPTX text for processing."""
        textContent = re.sub(r'\n{3,}', '\n\n', textContent)
        textContent = re.sub(r'[ \t]+', ' ', textContent)
        # Remove slide markers for cleaner processing
        textContent = re.sub(r'<Slide \d+ - Start>', '', textContent)
        textContent = re.sub(r'<Slide \d+ - End>', '', textContent)
        return textContent.strip()

    def _chunk_text(self, textContent: str):
        """Chunk PPTX content for processing, maintaining slide structure."""
        ...
        return [{"title": f"Slide Chunk {i+1}", "content": c.strip()} for i, c in enumerate(chunklst)]
    ...
    def evaluate(self, textContent: str, document_title: str) -> dict:
        ...
        for i, chunk in enumerate(chunks):
            logger.info(f"Processing PPTX chunk {i+1}/{len(chunks)}")

            prompt = build_pptx_contentcheck_prompt(
                chunk=chunk["content"],
            ...
            if not cur:
                logger.warning(f"PPTX chunk {i+1} failed, skipping...")
                continue
            previous_result = cur
